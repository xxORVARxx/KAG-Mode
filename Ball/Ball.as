...
void onTick( CBlob@ this ) {
  ...
  //                               Is It In .cfg File, line 35:   bool shape_collides = yes   ????
  {
    Vec2f vel = this.getVelocity();
    f32 angle = vel.Angle();
...
void onCollision( CBlob@ _this, CBlob@ _other_blob, bool _is_solid ) {
  if( @_other_blob == null )
    return;

  CPlayer@ player_hit = _other_blob.getPlayer();
  if( player_hit != null )
    print("Collision! with Player: "+ player_hit.getUsername() +"!");
  else
    print("Collision! with: "+ _other_blob.getName() +"!");
}
...
{
  if( this is null || hitterBlob is null || @this == @hitterBlob )
    return 0;
  ...
  // Arrows Kill It!:
  if( customData == Hitters::arrow || customData == Hitters::bomb_arrow ||
      customData == Hitters::water_stun || customData == Hitters::fire ) {
    return 10;
  }
  return damage * 0.5;
}
...
void onCollision_or_not_xx( CBlob@ _this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
  //-----------------------------------
  bool xx_just_return_from_here = true;
  if( xx_just_return_from_here )
    return;
